...
def update_db(spreadsheet_id,range_):
    ...
    for item in values:
        ...
        if date == '':
            ...
            "last update" = DEFAULT 
            WHERE id = %s;
            ''', (value, shares, price_currency, companies_id,))
        else:
            cur.execute ('''
            UPDATE companies
            SET 
            ...
            "last update" = %s 
            WHERE id = %s;
            ''', (value, shares, price_currency, date, companies_id,))
    ...
    print(len(companies_inserted),"companies updated")


def clear_sheet_updates(spreadsheet_id,range_):
...
def register_last_update(spreadsheet_id,range_):
    ...
    for item in values:
        if row == 1:
            ...
            row += 1
    duplicates = []
    for item in ticker_row.keys():
        if item in companies_inserted.keys():
            duplicates.append(item)
    ## here we register the update in the sheet
    sheet = service.spreadsheets()
    value_input_option = 'USER_ENTERED'
    for item in duplicates:
        row = ticker_row[item] 
        tmp_range = create_range(range_,row)
        cur.execute('''
        SELECT "last update" FROM companies WHERE "name" = %s''', (companies_inserted[item],))
        date = cur.fetchone()[0] 
        value_range_body = {
        "range": tmp_range,
        "majorDimension": 'ROWS',
